...
import { Searchbar, Chip, IconButton, Menu, Portal, Modal } from 'react-native-paper';
...
const ArtistListingScreen = () => {
  ...
  // Fetch artists
  const { data, loading: artistsLoading, error, refetch } = useQuery(GET_ARTISTS, {
    variables: queryVariables,
    onError: (err) => console.error('Error fetching artists:', err),
  ...
  const hasActiveFilters = selectedCategory || selectedSubCategories.length > 0 || sortBy || minBudget || maxBudget;
    ...
    <>
      ...
      <Animated.View style={[styles.header, { height: headerHeight, paddingTop: insets.top }]}>
        <Animated.View style={[styles.headerContent, { opacity: headerOpacity }]}>
          ...
          <Text style={styles.welcomeText}>Discover talented artists</Text>
          <Searchbar
      ...
      {showFilterChips && hasActiveFilters && (
        <Animated.View style={[styles.floatingChips, { top: insets.top + 40, opacity: floatingChipsOpacity, transform: [{ translateY: floatingChipsTranslateY }] }]}>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filterChipsScroll}>
            ...
            {currentLocation && (
              <Chip selected onClose={() => setCurrentLocation('Mumbai')} style={styles.filterChip} textStyle={styles.miniChipText}>
                {currentLocation}
              </Chip>
            )}
            {sortBy && (
              <Chip selected onClose={() => setSortBy(null)} style={styles.filterChip} textStyle={styles.miniChipText}>
                {sortBy === 'top_rated' ? 'Top Rated' : sortBy === 'popularity' ? 'Most Popular' : 'Most Booked'}
              </Chip>
            )}
...
const styles = StyleSheet.create({
  ...
  artistListContainer: { paddingHorizontal: 4, paddingBottom: 16 },
  emptyContainer: {
    flex: 1,
