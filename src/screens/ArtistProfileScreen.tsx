// src/screens/ArtistProfileScreen.tsx
...
import { GET_ARTISTS, GET_ARTIST_PORTFOLIO } from '../graphql/queries';
...
const COLORS = {
  category: { bg: '#4C68FF', text: '#FFFFFF' },
  artistType: { bg: '#FFB448', text: '#FFFFFF' },
  subCategory: { bg: '#E6F2FF', text: '#0066CC' },
};
...
export const ArtistProfileScreen = ({ route, navigation }: ArtistProfileScreenProps) => {
  ...
  const onStateChange = useCallback((state: string) => {
    if (state === 'playing') setLoadingMedia(false);
  }, []);
  ...
  const isInstaUrl = (url: string) => url.includes('instagram.com');
    ...
    return (
      <View style={styles.portfolioGrid}>
        {portfolioItems.map((item: PortfolioItem) => (
          ...
          >
            <Image source={{ uri: item.thumbnail || item.mediaUrl }} style={styles.portfolioThumbnail} />
        ...
        <View style={styles.header}>
          <View style={styles.headerTop}>
            <View style={styles.imageContainer}>
              <Avatar.Image size={100} source={{ uri: artist.profilePicture }} style={styles.profileImage} />
            </View>
          ...
          <View style={styles.socialMediaContainer}>
            ...
            <View style={styles.socialMediaIcons}>
              {artist.youtubeDisplay && artist.youtubeId && (
                <TouchableOpacity
                  onPress={() => Linking.openURL(getSocialUrl('youtube', artist.youtubeId)!)}
                  style={styles.socialIcon}
                >
                  <FontAwesome5 name="youtube" size={24} color="#FF0000" />
                </TouchableOpacity>
              )}
              {artist.instagramDisplay && artist.instagramUsername && (
                <TouchableOpacity
                  onPress={() => Linking.openURL(getSocialUrl('instagram', artist.instagramUsername)!)}
                  style={styles.socialIcon}
                >
                  <FontAwesome5 name="instagram" size={24} color="#E1306C" />
                </TouchableOpacity>
              )}
              {artist.facebookDisplay && artist.facebookId && (
                <TouchableOpacity
                  onPress={() => Linking.openURL(getSocialUrl('facebook', artist.facebookId)!)}
                  style={styles.socialIcon}
                >
                  <FontAwesome5 name="facebook" size={24} color="#1877F2" />
                </TouchableOpacity>
              )}
              {artist.xDisplay && artist.xUsername && (
                <TouchableOpacity
                  onPress={() => Linking.openURL(getSocialUrl('x', artist.xUsername)!)}
                  style={styles.socialIcon}
                >
